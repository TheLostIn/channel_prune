...
def prune_resnet18_conv_layer(model, layer_index, filter_index):
    ...
    new_down_conv = None
    conv = model.features._modules.get(str(layer_index)) #initial conv
    if layer_index == 0:
    		# self.model.features._modules.get(str(layer))[kt].conv1(x)
        conv = model.features._modules.get(str(layer_index))
        next_conv =  model.features._modules.get(str(4))[0].conv1
    ...
    if layer_index > 4 and layer_index < 9:
                ...
                if pt==1:
                        conv = model.features._modules.get(str(3+tt))[kt].conv1
                        next_conv =  model.features._modules.get(str(3+tt))[kt].conv2
                        #downout_conv =  model.features._modules.get(str(3+tt))[0].downsample[0]
                else:
                        if kt==0:
                                conv = model.features._modules.get(str(3+tt))[kt].conv2
                                next_conv =  model.features._modules.get(str(3+tt))[kt+1].conv1
            ...
            if pt==1:
                ...
                model.features._modules.get(str(3+tt))[kt].conv2 = next_new_conv
                    ...
                    #ds = torch.nn.Sequential(
                        #*(replace_layers(model.features._modules.get(str(3+tt))[0].downsample, i, [0], \
                            #[new_down_conv]) for i, _ in enumerate(model.features._modules.get(str(3+tt))[0].downsample)))
                    #model.features._modules.get(str(3+tt))[kt].downsample = ds
                    #model.features._modules.get(str(3+tt))[kt].downsample[0] = next_downin_conv
            else:
                if kt==0:
                    ...
                    #model.features._modules.get(str(3+tt+1))[0].downsample[0] = next_downin_conv
